...
fn bench_file_receiver(c: &mut Criterion) {
    ...
    c.bench_function("receive & decrypt 100k", |b| b.iter_custom(|iters| {
    ...
    let backup = stream.clone();
    c.bench_function("receive & decrypt 1M", |b| b.iter_custom(|iters| {
        let mut total_time = Duration::ZERO;
        for _i in 0..iters {
    ...
    let backup = stream.clone();
    group.bench_function("receive & decrypt 100M", |b| b.iter_custom(|iters| {
        let mut total_time = Duration::ZERO;
        for _i in 0..iters {
